window.onload = async () => {
  ...
  document.querySelector("#album-cover").innerHTML = `
            ...
            <img src="${
              albumArray.cover
            }" alt="" id="cover-img" class="shadow m-4 " />
            ...
            <div id="album-info" class="ps-4 mt-5 text-dark">
                ...
                <p>
                    ...
                    <a href=./artist.html?id=${albumArray.artist.id}>${
    albumArray.artist.name
  }</a> - ${albumArray.release_date.substring(0, 4)} - ${
    albumArray.nb_tracks
  }, ${secToMin(albumArray.duration)} min</span>
                </p>
            </div>`;
    ...
    trackDiv.innerHTML += `
        <div class="title-head text-light row mx-2 mt-4 track-div-box">
                            <div class="col row justify-content-start ">
                                <div class="col-2">
                                    <h6>${
                                      albumArray.tracks.data.indexOf(t) + 1
                                    }</h6>
                                </div>
                    
                                <div class="col links-of-list">
                                    <div class="col song-to-play"><a class="song-name" onclick="myFunction('${
                                      t.title
                                    }','${t.preview}')">${t.title}</a></div>
                                    <div class="col song-to-play text-white-50"><a href=./artist.html?id=${
                                      t.artist.id
                                    }>${t.artist.name}</a></div>
                                </div>
                            </div>
                            <div class="col-2 text-center text-white-50">${
                              t.rank
                            }</div>
                            <div class="col-2 text-center text-white-50">${secToMin(
                              t.duration
                            )} min </div>
                        </div>`;
  }
